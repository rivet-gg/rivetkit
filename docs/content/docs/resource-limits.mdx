...
| `sqlite.maxInFlightRequests` | Concurrent actor-UDS SQLite requests admitted for one VM | Default is `64`. Requests above the cap fail with a typed error naming this field. |
| `sqlite.maxQueuedRequestBytes` | Actor-UDS SQLite request bytes waiting to be written | Default is `64 MiB`. Requests above the cap fail instead of growing the client queue without bound. |
