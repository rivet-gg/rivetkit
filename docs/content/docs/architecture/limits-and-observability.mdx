...
runtime → a per-execution frame channel → the V8→host event channel → the sidecar
...
- [Resource Limits](/agentos/docs/resource-limits) — the full `limits` config surface.
- [Processes](/agentos/docs/architecture/processes) and
  [Persistence](/agentos/docs/persistence) — the layers the queue chain runs
  through.
